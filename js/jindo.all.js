 ...
 * @param {String}   option.method	  HTTP 요청에 사용할 방식. "post" 혹은 "get"을 이용한다. 단, 특정 type에서 요청 방식이 강제되는 경우가 있다. 예를 들어 type을 jsonp으로 설정했다면 method는 강제적으로 get이 된다. 기본값은 "post"이다.
...
jindo.$Ajax.Queue.prototype._requestSync = function (nIdx, oParam) {
    ...
    if (this.option('useResultAsParam') && oParam) {
        try {
            for (var x in oParam)
                if (typeof _oParam[x] == 'undefined') _oParam[x] = oParam[x]
        } catch (e) {};
    }
